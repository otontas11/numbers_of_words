...
- `activity_word_path_game.xml` ve `WordWheelView` yalnız oyun ekranının yerleşim ve görsel diline referanstır. Android projesindeki kelime üretimi, skor, ipucu kredisi, reklam, tur ve oyun kuralları alınmaz; NOW matematik motoru, mücevher ekonomisi ve callback akışı değiştirilemez.
...
6. Tüm **ana hedefler** bitince bonus kartının durumuna bakılmadan kutlama ve seviye geçişi çalışır. Ara puzzle’da kompakt/kısa konfeti vardır, `levelComplete` çalmaz ve 1 sn bekleme tabanı yoktur. Destinasyon bitişi, Country Challenge girişi ve ülke bitişinde tam tören vardır: son hedef `success`, konfeti anında `levelComplete`, ardından puan uçuşu. Çözülmemiş bonus sonraki seviyeye geçişi hiçbir zaman engellemez.
7. Her ülkede üç destinasyon da 5&apos;er puzzle içerir; 16. puzzle Country Challenge&apos;dır.
8. Country Challenge tamamlanınca vize pulu ve landmark kazanılır. Ülke modalında birincil eylem sonraki ülkeye/puzzle’a devamdır; ikinci eylem `Haritayı gör` ile Seyahat ekranını açar. Günlük meydan okuma ve kayıtlı ana tur tahtası bu kısayolla ezilmez. Geri tuşu varsayılanı ana menüdür.
...
10. İlk oturumda tam eğitim varsayılandır. Modalda görünür **Atla** `tutorial-completed = done` yazar ve kapanır. Eğitim pratikleri toplama, çıkarma, çarpma, bonus ve kısa 2 adımlı tam sayı bölmedir. Eğitim kapanışında `levelComplete` + konfeti vardır; gerçek tahta ikinci kutlama çalmaz. İşlem yönlendirmesi eğitim bitene kadar gösterilmez. Eğitimden sonra her destinasyon/şehir değişiminde (`locationLevel === 1`) ve Country Challenge girişinde çarkın ortasında işlem sembolü (`+ − × ÷`) bir kez görünür; oyuncu ilk sayıya dokununca kapanır. Aynı şehirdeki sonraki puzzle’larda tekrarlanmaz. İşlem türü başına global 2-kez sayacı yoktur. Gösterilen `locationId` listesi `@numbers-of-wonders/operation-guide-v1` ile saklanır; reklam veya ekran değişimi sıfırlamaz.
...
| 1. ülke, puzzle 11–16 | 6 | 4 | İki adımlı toplama; Challenge dahil |
...
- Adaptif zorluk, aktif oyun süresi, hata ve ipucu geçmişiyle yeni şehir başında `−1/0/+1` sayı zorluğu uygular. Aynı şehirde art arda iki “zorlandı” sonrası (Challenge hariç) sonraki puzzle en fazla 1 kademe rahatlar; işlem türü ve temel adım kuralı değişmez. İlk beş öğretici ülkede adaptif kapalıdır; 15 saniyelik hareketsizlik aktif süreden çıkarılır. Ayrıntılı sözleşme `ADAPTIVE_DIFFICULTY.md` içindedir.
...
Katalog tam olarak 24 ardışık rota, 166 benzersiz ülke/170 ülke etabı, 510 gerçek destinasyon ve 2.720 ana puzzle içerir:
...
- Her ülke tam `16` ana puzzle içerir: destinasyonlar `1–5`, `6–10`, `11–15`; Country Challenge `16`.
...
- Ana menünün ve oyun HUD&apos;unun sol üst grubunda oyuncu puanı, sağ üst grubunda mücevher bakiyesi bulunur. Yeni oyuncu `30` mücevherle başlar; ipucu `10` mücevherdir; yeni rota mührü `+10` mücevherdir. Ayrı ipucu kredisi bakiyesi yoktur. Bölüm tamamlandığında, yapılan ana hedef işlemlerindeki sayıların toplamı × adım sayısı kadar puan toplu olarak eklenir. Bonus hedef ödülü sonuçtan bağımsız olarak 2/3/4 adım için `4/8/14`, Challenge&apos;da `6/12/21` mücevherdir. İlk kez bulunan serbest Bonus Keşif `+1 mücevher` verir. Toplam puan ve mücevher bakiyesi kalıcı progress&apos;te saklanır.
- Ekranın merkezindeki büyük altın çerçeveli `BÖLÜM` düğmesi aktif ülke/destinasyon içindeki puzzle numarasını gösterir ve kayıtlı sayı matrisi ile çözülmüş hedeflerden **tek dokunuşla** devam eder. Ana menüde Günlük Meydan Okuma kartı streak ve bugünkü ödül/durumu gösterir; tamamlandıktan sonra da tekrar oynanabilir. Alt `GÖREVLER` aynı ekranı açar. Daily kendi progress storage’sunu kullanır; 5 yükselen puzzle aynı gün aynı hedeflerle üretilir, ödül mücevheri yalnız ilk claim’de ana bakiyeye eklenir, ana tur tahtasını ezmez.
...
- Devam kartı hero üzerine `16 dp` biner, yatayda `16 dp` boşluk bırakır ve `18 dp` köşe yarıçapı kullanır. Başlık `OYUNA DEVAM ET`; alt satır aktif ülke, destinasyon ve destinasyon içi puzzle ilerlemesidir. Kartın ilerleme çizgisi aktif ülkenin `x/16` durumunu gösterir.
...
- Şeridin ilk satırı ülke/flag, aktif işlem çipi ve ülke içi `x/16` ilerlemesini gösterir. Challenge aktifken yalnız ülke adının sağında, layout değiştirmeyen hafif pulse animasyonlu `CHALLENGE` etiketi görünür; oyun alanının başka bir yerinde tekrar edilmez.
- İkinci satır üç destinasyonu `✓ tamamlandı / ● aktif / ○ bekliyor` semantiğiyle sıralar. İlerleme rayı puzzle ağırlıklarıyla `5 + 5 + 5 + 1` oranında dört parçadır; son yaklaşık `%5` genişlikteki kupa parçası Country Challenge ilerlemesidir.
...
- Bir destinasyonun son puzzle&apos;ı tamamlandığında otomatik seviye değişmeden önce ortada `DESTİNASYON TAMAMLANDI` kartı görünür; biten destinasyon ve açılan sıradaki destinasyon/Challenge açıkça yazılır. Bu kart bilgi amaçlıdır ve destinasyon geçişinde yaklaşık `1.15 sn`, Challenge girişinde yaklaşık `2.1 sn` sonra akış devam eder.
- Country Challenge tamamlandığında sonraki ülkeye otomatik ve sessiz geçilmez. Üç destinasyonun onay işaretlerini, `16/16`, pasaport damgasını, landmark ödülünü ve açılan sonraki ülkeyi gösteren zorunlu ülke tamamlama modalı açılır. Yeni ülkenin ilk puzzle&apos;ı birincil devam eylemiyle başlatılır; `Haritayı gör` Seyahat ekranını açar. Tamamlanmış kayıt uygulama yeniden açıldığında da bu modal atlanmaz.
...
- Ana turda bir ülkede puzzle `n → n+1` geçişinde çark doğumu ekranda görünür. Ara puzzle: son hedef uçuşu ve kısa kutlama biter, overlay kapanır, eski sayılar merkeze toplanır (`220 ms`, kaybolmadan), yeni sayılar yörüngeye doğar (`460 ms`); sonra çizim serbesttir. Destinasyon kartı varsa kart kapandıktan sonra aynı outro+intro oynar; kartın arkasında sessiz token yoktur. Ülke modalı kapanıp oyun görününce intro zorunludur (outro modal altında kalabilir). İlk Play ve kayıtlı devamda intro zorunlu değildir. Karıştırma bu doğumu ezmez.
...
- `hint.wav` / `pop_hint.wav`: İpucu düğümü, `620 → 868 Hz`, aynı `80 ms` pop zarfı; oynatma başı `0`.
- `shuffle.wav` / `pop_shuffle.wav`: Karıştır düğümü, `360 → 504 Hz`, aynı `80 ms` pop zarfı; pembe gürültü kullanılmaz; oynatma başı `0`.
...
- `points.wav`: tam törende `levelComplete` bittikten sonra puan uçuşu tiki. `game-treasure.wav` ile aynı ses değildir.
...
- Kaynak HTML son hedefte `success` dışında ikinci bir ses çalmaz; `400 ms` sonra bölüm konfettisi başlar. Native sürümde son hedef `success`, destinasyon/Challenge/ülke konfetisiyle aynı anda `game-treasure.wav` (`levelComplete`, `%35`) çalar; puan uçuşu `320 ms` sonra `points.wav` kullanır. Ara puzzle kompakt kutlamada `levelComplete` yoktur. `levelComplete` olayı makro titreşimi de korur.
- Arka plan müziği kutlama, ülke modalı ve destinasyon kartında `0.4` duck alır; aç/kapa ve ön plan geçişinde `250–400 ms` aralığında `320 ms` fade kullanır. Arka planda çalmaz.
- Sesler Expo SDK 57 `expo-audio` oyuncularıyla önceden yüklenir, tekrar öncesi başa sarılır, diğer uygulama sesini kesmez ve arka planda çalmaz. Android native build’de kısa efektler SoundPool üzerinden gider.
...
Seviye motoru tüm `1–2720` ana turu ve Master Tour başlangıcını tekrarlı rastgele koşularla doğrular: 24 rota, 166 benzersiz ülke/170 ülke etabı, 510 destinasyon, `5+5+5+Challenge`, doğru düğüm/hedef adedi, benzersiz hedefler, çözülebilirlik, adım tutarlılığı ve ardışık hedef tekrarsızlığı.
...
Bağlı fiziksel Android veya iOS cihazlarda açılış, kalıcı ilerleme, sürükleme, çeper çizgisi, ipucu, yaylı karıştırma, ses aç/kapat, mikro başarı, bölüm sonu konfeti ve iki modal kontrol edilir. Ana menüdeki Günlük Keşif giriş noktası kabul kapsamındadır. Time Attack, PvP ve 3D pasaport mevcut sürümün kabul kapsamına dahil değildir.
