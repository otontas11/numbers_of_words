...
function ToggleRow({
  ...
  return (
    <Pressable
      accessibilityLabel={`${title}, ${enabled ? 'açık' : 'kapalı'}`}
  ...
  return (
    <BlurView
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.modalSubtitle}>Sayı yolculuğunda kullanmak istediğin sesleri seç.</Text>

        <ToggleRow
          enabled={effectsEnabled}
          icon="🔊"
          onChange={onEffectsChange}
          subtitle="Sayı, cevap ve başarı sesleri"
          title="Ses"
        />
        <ToggleRow
          ...
          subtitle="Sakin yolculuk arka plan müziği"
          title="Müzik"
        />

        <View style={[styles.volumeRow, !musicEnabled && styles.volumeDisabled]}>
          <View style={styles.volumeHeader}>
            <Text style={styles.volumeTitle}>Müzik seviyesi</Text>
            <Text style={styles.volumeValue}>{volumePercent}%</Text>
          </View>
          <Slider
            accessibilityLabel={`Müzik seviyesi yüzde ${volumePercent}`}
